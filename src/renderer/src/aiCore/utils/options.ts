...
export function buildProviderOptions(
   ...
   *    - Otherwise → map to primary (this is a proxy provider like newapi)
   * 3. Otherwise → treat as regular parameter, merge to primary provider
   *
   * Example:
   * - User writes `newapi: { opt: 'val' }` → mapped to `google: { opt: 'val' }` (case 2, proxy)
  ...
  for (const key of Object.keys(providerParams)) {
    if (actualAiSdkProviderIds.includes(key)) {
      ...
      if (key === SystemProviderIds.gateway) {
        ...
        // Proxy provider - map to the actual AI SDK provider
        providerSpecificOptions = {
          ...providerSpecificOptions,
...
> {
  // Proxy providers such as NewAPI may annotate model.endpoint_type to force a specific protocol.
